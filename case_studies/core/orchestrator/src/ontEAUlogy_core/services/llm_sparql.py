...
class SparqlTranslationResult(BaseModel):
    ...
    simulation_required: bool = Field(default=False, description="Whether simulation is needed")
    suggested_models: List[str] = Field(default_factory=list, description="Relevant model IDs")
...
class LLMService:
    ...
    async def initialize(self) -> bool:
        ...
        if self._provider == "auto":
            ...
            if await self._try_initialize_lmstudio():
                return True
            logger.warning("LM Studio not available. Install LM Studio and load a model.")
            return False
    ...
    async def _try_initialize_openrouter(self) -> bool:
        ...
        api_key = self._api_key or os.environ.get("OPENROUTER_API_KEY") or settings.llm_api_key
        ...
        try:
            self._model = self._model or self.PROVIDER_CONFIGS["openrouter"]["default_model"]
            ...
            if "Connection" in str(e) or "Network" in str(e):
                logger.error("Cannot connect to OpenRouter. Check your network connection.")
    ...
    async def translate(self, question: str) -> SparqlTranslationResult:
        ...
        while attempt < self._max_retries:
            ...
            try:
                ...
                try:
                    translation = SparqlTranslationResult.model_validate_json(raw_llm_output)
                    ...
                    error_file = (
                        DEBUG_DIR / f"parse_error_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
                    )
                    ...
                    last_error = (
                        f"LLM returned invalid JSON: {json_e}. Output saved to {error_file}"
                    )
                    if is_final:
                        return SparqlTranslationResult(
                            generated_sparql="",
...
"""
    ...
    def _find_suggested_models(self, question: str) -> List[str]:
        ...
        if "wwtp1" in question_lower or ("wwtp" in question_lower and "1" in question):
            models.append("wwtp1")
        elif "wwtp2" in question_lower or ("wwtp" in question_lower and "2" in question):
            models.append("wwtp2")
